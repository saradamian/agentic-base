...
| **security** against misuse of the blocks | the runs service refusing any data request without a token that lists the tenant; outbound URL check with DNS pinning; the structural pre-filter on generated code; curated surfaces so a block publishes a reviewed subset; token redaction on error strings | publishes only its curated surface; scopes it by the caller's identity; carries the platform's isolation tier in its profile | SURFconext and SRAM; secret management; network policy and admission control; the container image scan; Dependency Track |
...
|---|---|---|---|
| personal data in a transcript is written as sent | `redaction` on every run: `none`, or the instrument, its mode and its model revision. The service redacts on write when configured, patterns always and names from a model on Willma with GLiNER as the fallback, refuses the write when neither can run, and `extra.redaction` says how many strings were examined, what was found and which instrument ran. A transcript with `none` and a personal classification is a finding | a deployment turning it on, with a model for names and places: `redaction.md` has the choice and its measured cost | T2.6, the LLM sandbox with safety filters and logging; T4.4 data governance |
