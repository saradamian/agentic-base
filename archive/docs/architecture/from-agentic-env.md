...
| `agentic_base.domain.epochs` | `agentic/core/component_versions.py` and its ledger reader | decided 2026-09-14: agentic-env's model is the rule. Runs pool only when every recorded version is in one set declared equivalent, and a version nobody declared is refused; the boundary model placed any later release on the new side unreviewed and could not place runs recorded before versions were. `VersionEpochs` here is that model, returning a verdict instead of raising. The commit boundary for an application's own code stays, with no counterpart there | agentic-env keeps `EQUIVALENT_EPOCHS` as its declarations and replaces `epoch_of` and `assert_poolable` with `VersionEpochs(...).check`, once no campaign is importing the installed package |
