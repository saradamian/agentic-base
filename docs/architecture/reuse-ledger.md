...
exists, is maintained, and models the thing — not on an impression. "Feels commodity" is not a
...
## SURF internal — reuse before anything external
...
| secrets | SDP secret management, per its guide | ADOPT — and it is the fix for secrets-in-job-scripts |
