...
| site identifiers in a tree that is about to be published | nothing. gitleaks reads credentials, the redaction layer reads personal data, and a hostname, a home directory or an organisation slug is neither | BUILD, and deliberately small: `tests/test_public_hygiene.py`, by shape rather than by word list, with a planted example per detector. It calls the redaction layer's `PatternDetector` for the checksummed money and identity shapes rather than carrying a second copy, and leaves credentials to gitleaks, which also reads the history. People's names are in no gate: the only detector for them needs a model, and a gate that goes permissive when an endpoint is unreachable is worse than none. Revisit when a maintained scanner takes an allow-list of hosts a repository may name |
...
| **comparison validity** | no experiment tracker adjudicates whether a contrast is sound, and we found no reporting standard in agent evaluation that asks for the per-arm accounting that would show it. The check is ours; the accounting is CONSORT's flow diagram, and the decision is a score interval on each exclusion-rate difference — per channel, pooled so free-text channel names cannot dilute a gap, and over items an arm never attempted — with "too little data" as its own verdict. The flow diagram and the interval are both adopted as standards below rather than reinvented | any tracker ships arm-correlated missingness detection, or an agent-evaluation reporting standard mandates a per-arm exclusion flow |
...
| the interval on a rate difference | **Wilson (1927)**, the score interval for one proportion; **Newcombe (1998) method 10**, the square-and-add combination of two of them for a difference | ADOPT the published formulas. `wilson_interval` and `newcombe_interval` implement them in plain `math`, because the library half installs without numpy or scipy, and the tests hold them to the values printed in Newcombe's Table II |
...
| `migrations/schema.py` | the schema versioned with Alembic, shipped in the package, applied by `agentic-base-migrate`, and checked on start: a local SQLite file is migrated, any other database must already be current | ADOPT Alembic. The start-time refusal is ours, because Alembic migrates when asked and says nothing when nobody asks |
...
| `adapters.py` | JSONL rows and Inspect AI eval logs (the `.json` document and the `.eval` zip) read into the shape `domain/validity` checks and `domain/outcomes` judges, standard library only; a record with no readable verdict becomes the `no_verdict` exclusion channel rather than an analysed run | BRIDGE. The log schema is Inspect's, read with `json` and `zipfile` rather than by importing `inspect_ai`, which does not install on the library's 3.10 four-dependency floor — an adapter that costs the floor is a second copy of the problem, not an adapter. Revisit when Inspect publishes a dependency-light log reader to import instead |
| `cli.py` | `agentic-base check`: the validity verdict and the citability summary over logs the caller already has, exit-coded so it gates a CI job — 0 only when the check both passed and could have flagged | BUILD, and deliberately thin: every decision is the domain's, the command only reads files and prints. Revisit when an experiment tracker's CLI adjudicates a contrast, at which point point this command at it |
| `demo.py` | the uneven-timeout scenario generated in memory and printed beside the checked verdict, shipped inside the wheel so `python -m agentic_base.demo` works from a bare install | BUILD, a worked example held to its output by `tests/test_demo.py`. Revisit when the runnable examples ship in the distribution some other way |
...
| `retention_sweep.py` | the retention policies per tenant, the sweep that erases due transcripts and logs each erasure, and an erasure for one request, as `agentic-base-retention` | BUILD, over the domain's rules. Revisit when the platform offers retention as a tenant resource that can be told which fields are personal |
...
| `domain/epochs.py` | declaring that a revision changed a field's meaning, and which versions of an imported component are equivalent, and refusing to pool across either | BUILD. No tracker records a dependency version as a pooling key. Revisit when one does |
