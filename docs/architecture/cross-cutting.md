...
|---|---|---|---|
| personal data in a transcript is written as sent | `redaction` on every run: `none`, or the instrument, its mode and its model revision. The service redacts on write with Presidio when configured, and `extra.redaction` says how many strings were examined and what was found. A transcript with `none` and a personal classification is a finding | a deployment turning it on, with a model for names and places: `redaction.md` has the choice and its measured cost | T2.6, the LLM sandbox with safety filters and logging; T4.4 data governance |
