...
| **redaction** | personal data removed from a transcript before it is written, with the instrument named | Presidio in the `redaction` extra, applied by the service on write when configured; `redaction` on every run names the instrument, its mode and its model | a deployment choosing a model and the hardware for it (`redaction.md`); redaction before a prompt reaches a model, on the agent's side |
