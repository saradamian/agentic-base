...
| **redaction** | personal data removed from a transcript before it is written, with the instrument named | patterns in the library, names from a model on Willma with a GLiNER fallback, applied by the service on write and failing closed; `redaction` on every run names what ran | a deployment choosing a model and the hardware for it (`redaction.md`); redaction before a prompt reaches a model, on the agent's side |
| **transparency** | a person is told they are dealing with an AI, and generated content is marked machine-readably. The AI Act's article 50, in force since 2 August 2026 | `disclosure` and `content_marking` on every run, both `none` until something sets them | the channels block saying it on every outbound message and recording that it did; a marking format for text, which does not exist in the open the way Content Credentials do for images |
